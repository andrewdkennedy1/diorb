...
use crate::models::{BenchmarkResult, LatencyStats, PerformanceMetrics};
...
impl WorkerInfo {
    ...
    pub fn is_completed(&self) -> bool {
        matches!(
            self.status,
            WorkerStatus::Completed | WorkerStatus::Failed(_) | WorkerStatus::Cancelled
        )
    }
}
...
impl WorkerManager {
    ...
    async fn spawn_workers(&self, progress_tx: mpsc::Sender<AggregatedProgress>) -> Result<()> {
        ...
        for worker in workers.iter_mut() {
            ...
            let handle = match self.config.mode {
                BenchmarkMode::SequentialWrite | BenchmarkMode::SequentialRead => {
                    self.spawn_sequential_worker(worker.id, worker_tx, cancel_rx)
                        .await?
                }
                BenchmarkMode::RandomReadWrite => {
                    self.spawn_random_worker(worker.id, worker_tx, cancel_rx)
                        .await?
                }
                BenchmarkMode::Mixed { read_ratio } => {
                    self.spawn_mixed_worker(worker.id, read_ratio, worker_tx, cancel_rx)
                        .await?
                }
            };
        ...
        self.start_progress_aggregation(worker_progress_receivers, progress_tx)
            .await;
    ...
    async fn spawn_sequential_worker(
        ...
        let handle = tokio::spawn(async move {
            ...
            if cancel_rx.try_recv().is_ok() {
                return Err(DIOrbError::BenchmarkError(
                    "Worker cancelled before start".to_string(),
                ));
            }
    ...
    /// Spawn a random I/O benchmark worker
    async fn spawn_random_worker(
        ...
        let benchmark = crate::bench::random::RandomBenchmark::new(worker_config)?;
        ...
        let handle = tokio::spawn(async move {
            ...
            tokio::select! {
                result = benchmark.run(0.5, progress_tx) => result,
                _ = cancel_rx => Err(DIOrbError::BenchmarkError(format!("Worker {} cancelled", worker_id))),
            }
        });
    ...
    /// Spawn a mixed I/O benchmark worker
    async fn spawn_mixed_worker(
        ...
        let handle = tokio::spawn(async move {
            ...
            tokio::select! {
                result = benchmark.run(read_ratio, progress_tx) => result,
                _ = cancel_rx => Err(DIOrbError::BenchmarkError(format!("Worker {} cancelled", worker_id))),
            }
        });
    ...
    fn aggregate_progress(
        worker_progress: &[Option<ProgressUpdate>],
        start_time: Instant,
    ) -> AggregatedProgress {
        ...
        let eta = if total_bytes_processed > 0 && total_throughput > 0.0 {
            ...
            Some(Duration::from_secs_f64(
                remaining_bytes as f64 / rate_bytes_per_sec,
            ))
        } else {
            None
        };
            ...
            if let Some(handle) = worker.handle.take() {
                match handle.await {
                    ...
                    Err(e) => {
                        ...
                        return Err(DIOrbError::BenchmarkError(format!(
                            "Worker join failed: {}",
                            e
                        )));
                    }
        ...
        let combined_latency = if !all_latency_samples.is_empty() {
            ...
            let avg = Duration::from_nanos(
                (all_latency_samples
                    ...
                    .map(|d| d.as_nanos())
                    .sum::<u128>()
                    / all_latency_samples.len() as u128) as u64,
            );

            let mut percentiles = std::collections::HashMap::new();
            percentiles.insert(
                50,
                all_latency_samples[all_latency_samples.len() * 50 / 100],
            );
            percentiles.insert(
                95,
                all_latency_samples[all_latency_samples.len() * 95 / 100],
            );
            percentiles.insert(
                99,
                all_latency_samples[all_latency_samples.len() * 99 / 100],
            );
...
mod tests {
    ...
    async fn test_single_worker_sequential_benchmark() {
        ...
        while let Some(update) = timeout(Duration::from_secs(5), progress_rx.recv())
            .await
            .ok()
            .flatten()
        {
    ...
    async fn test_multiple_worker_sequential_benchmark() {
        ...
        // Collect progress updates
        let mut updates = Vec::new();
        while let Some(update) = timeout(Duration::from_secs(10), progress_rx.recv())
            .await
            .ok()
            .flatten()
        {
    ...
    async fn test_unsupported_benchmark_modes() {
        ...
        let config =
            BenchmarkConfig::random_read_write().with_disk_path(temp_dir.path().to_path_buf());
        ...
        let config = BenchmarkConfig::mixed(0.7).with_disk_path(temp_dir.path().to_path_buf());
